...
MIN_VOICE_DURATION = 5  # 5 seconds minimum
MAX_VOICE_DURATION = 10  # 10 seconds maximum
ALLOWED_AUDIO_TYPES = ["audio/mpeg", "audio/wav", "audio/x-wav", "audio/mp4", "audio/x-m4a", "audio/m4a"]
ALLOWED_AUDIO_EXTENSIONS = [".mp3", ".wav", ".m4a"]

# Mock safety filter - in production, this would use an AI moderation service
async def check_voice_intro_safety(audio_data: bytes, filename: str) -> dict:
    """
    Mock safety filter for voice intros.
    In production, this would use an AI service to check for:
    - Offensive language
    - Inappropriate content
    - Harmful speech
    
    For testing, filenames containing 'unsafe' or 'flagged' will be rejected.
    """
    is_safe = True
    reason = None
    
    # Mock: flag files with 'unsafe' or 'flagged' in the filename for testing
    if filename and ('unsafe' in filename.lower() or 'flagged' in filename.lower()):
        is_safe = False
        reason = "Content flagged by safety filter"
    
    # Mock: Randomly flag ~5% of uploads for realistic testing (disabled for predictability)
    # import random
...
@api_router.post("/profile/voice-intro")
async def upload_voice_intro(
    ...
    Returns URL to the stored voice intro on success.
    Returns specific error message if flagged by safety filter.
    """
    ...
    if file_ext not in ALLOWED_AUDIO_EXTENSIONS:
        raise HTTPException(
            status_code=400, 
            detail=f"Invalid file type. Please upload an MP3, WAV, or M4A file."
        )
    
    # Validate content type
    content_type = file.content_type or ""
    if content_type not in ALLOWED_AUDIO_TYPES:
        # Some browsers send different content types, so also check extension
        if file_ext not in ALLOWED_AUDIO_EXTENSIONS:
            raise HTTPException(
                status_code=400, 
    ...
    # Note: True duration validation would require audio processing libraries
    # like mutagen or pydub. For now, we estimate based on file size.
    # A typical voice recording at 128kbps is about 1MB per minute.
    # 5-10 seconds would be roughly 80KB-160KB at 128kbps.
    # We'll be lenient and accept files between 10KB and 10MB.
    if len(content) < 10 * 1024:  # Less than 10KB is likely too short
        raise HTTPException(
            status_code=400,
    ...
    # Generate unique voice intro ID
    ...
    # Store voice intro in database (base64 encoded)
    voice_intro_data = {
        "id": voice_intro_id,
        "user_id": current_user["id"],
...
@api_router.get("/profile/voice-intro/{voice_intro_id}")
