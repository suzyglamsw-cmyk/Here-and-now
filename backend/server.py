...
# JWT Config
JWT_SECRET = os.environ.get('JWT_SECRET', 'midnight-social-secret-key-2024-here-now-app-secure-fallback-v1')
JWT_ALGORITHM = "HS256"
...
async def get_people_not_here(
    radius: str = "0-10",  # "0-10", "10-25", or "25-50" miles
    current_user: dict = Depends(get_current_user)
):
    """
    Get people nearby within specified radius (Not Here mode).
    ...
    Radius options: 0-10 miles, 10-25 miles, 25-50 miles
    """
    ...
    for user in users:
        ...
        if is_self:
            ...
            i_revealed_to_them = False
            they_revealed_to_me = False
            is_connection_accepted = False
        else:
            # Check glance status
            has_glanced_at_me = await db.glances.find_one({
                "from_user_id": user["id"],
            ...
            # Check reveal status - BOTH must have pressed reveal
            i_revealed_to_them = await db.reveals.find_one({
                "revealer_id": current_user["id"],
                "revealed_to_id": user["id"]
            }) is not None
            ...
            # Connection accepted = mutual glance or connected
            is_mutual = has_glanced_at_me and i_glanced_at
            is_connection_accepted = is_mutual or is_connected
            
            # Revealed only when BOTH have pressed reveal
            is_revealed = i_revealed_to_them and they_revealed_to_me
    ...
    for other_user_id, thread_data in threads.items():
        ...
        if user:
            ...
            result.append({
                ...
                "photos": [f"/api/photos/serve/{p}" for p in photos if p] if photos else [],  # Full photos array for new avatar design
